...
function actualizarCabecerasDias() {
  ...
  diasVisibles.forEach((dia) => {
    ...
    header.className =
      `day-header ${dia === 5 ? "day-saturday" : dia === 6 ? "day-sunday" : ""}`.trim();
...
async function iniciarApp() {
  ...
  document
    .getElementById("btnDiaAnterior")
    ?.addEventListener("click", () => moverDiaVista(-1));
  document
    .getElementById("btnDiaSiguiente")
    ?.addEventListener("click", () => moverDiaVista(1));
...
function rellenarSelectoresHoras() {
  ...
  const inputs = [
    document.getElementById("inputHora"),
    document.getElementById("selectReagendaHora"),
  ];
...
function actualizarLabelSemana() {
  ...
  if (lbl)
    lbl.textContent = `${fechaLunesActual.getDate()} de ${meses[fechaLunesActual.getMonth()]} - ${finDeSemana.getDate()} de ${meses[finDeSemana.getMonth()]} (${fechaLunesActual.getFullYear()})`;
...
function actualizarEstadisticas() {
  ...
  if (document.getElementById("statSinSena"))
    document.getElementById("statSinSena").textContent = sinSena;
  if (document.getElementById("statConSena"))
