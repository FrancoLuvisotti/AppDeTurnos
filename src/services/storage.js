...
function turnoFijoActivoEnSemana(turno, fechaSemanaRef) {
  ...
  if (turno.fechaSemanaRef && fechaSemanaRef < turno.fechaSemanaRef)
    return false;
  if (
    turno.fechaUltimaSemanaFijo &&
    fechaSemanaRef > turno.fechaUltimaSemanaFijo
  )
    return false;
...
function crearRegistroFaltaDeFijo(turnoFijo, fechaSemanaRef) {
  ...
  return {
    ...
    sena: turnoFijo.sena,
    usaQuincho: turnoFijo.usaQuincho === true,
    senaQuincho: turnoFijo.senaQuincho || 0,
    fijo: false,
    falto: true,
    dia: turnoFijo.dia,
    hora: turnoFijo.hora,
...
function asegurarRegistroCliente(
  telefono,
  nombre,
  incrementoJugo = 0,
  incrementoFalto = 0,
  ...
  if (!clientesDB[telLimpio])
    clientesDB[telLimpio] = { nombre, jugo: 0, falto: 0 };
  clientesDB[telLimpio].nombre = nombre;
  clientesDB[telLimpio].jugo = Math.max(
    0,
    (clientesDB[telLimpio].jugo || 0) + incrementoJugo,
  );
  clientesDB[telLimpio].falto = Math.max(
    0,
    (clientesDB[telLimpio].falto || 0) + incrementoFalto,
  );
  guardarDatos();
}
...
function obtenerTurnoEn(dia, hora, fechaSemanaRef) {
  ...
  const fijoExacto = turnos.find(
    (t) =>
      t.fijo === true &&
      t.dia === dia &&
      t.hora === hora &&
      t.fechaSemanaRef === fechaSemanaRef &&
      turnoFijoActivoEnSemana(t, fechaSemanaRef),
  );
  if (fijoExacto) return fijoExacto;

  const fijo = turnos.find(
    (t) =>
      t.dia === dia &&
      t.hora === hora &&
      turnoFijoActivoEnSemana(t, fechaSemanaRef),
